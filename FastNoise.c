...
static int FastRound(FN_DECIMAL f) { return (f >= 0) ? (int)(f + 0.5) : (int)(f - 0.5); }
static int FastAbsI(int i) { return abs(i); }
...
unsigned char Index4D_32(unsigned char offset, int x, int y, int z, int w)
{
...
unsigned char Index4D_256(unsigned char offset, int x, int y, int z, int w)
{
...
FN_DECIMAL ValCoord2DFast(unsigned char offset, int x, int y)
{
	return VAL_LUT[Index2D_256(offset, x, y)];
}
FN_DECIMAL ValCoord3DFast(unsigned char offset, int x, int y, int z)
{
	return VAL_LUT[Index3D_256(offset, x, y, z)];
}

FN_DECIMAL GradCoord2D(unsigned char offset, int x, int y, FN_DECIMAL xd, FN_DECIMAL yd)
{
	unsigned char lutPos = Index2D_12(offset, x, y);

	return xd*GRAD_X[lutPos] + yd*GRAD_Y[lutPos];
}
FN_DECIMAL GradCoord3D(unsigned char offset, int x, int y, int z, FN_DECIMAL xd, FN_DECIMAL yd, FN_DECIMAL zd)
{
	unsigned char lutPos = Index3D_12(offset, x, y, z);

	return xd*GRAD_X[lutPos] + yd*GRAD_Y[lutPos] + zd*GRAD_Z[lutPos];
}
FN_DECIMAL GradCoord4D(unsigned char offset, int x, int y, int z, int w, FN_DECIMAL xd, FN_DECIMAL yd, FN_DECIMAL zd, FN_DECIMAL wd)
{
...
{
	FN_DECIMAL sum = SinglePerlin(m_perm[0], x, y, z, m_interp);
	FN_DECIMAL amp = 1;
	int i = 0;
	...
	while (++i < m_octaves)
	{
		...
		amp *= m_gain;
		sum += SinglePerlin(m_perm[i], x, y, z, m_interp) * amp;
	}
...
{
	FN_DECIMAL sum = FastAbs(SinglePerlin(m_perm[0], x, y, z, m_interp)) * 2 - 1;
	FN_DECIMAL amp = 1;
	int i = 0;
	...
	while (++i < m_octaves)
	{
		...
		amp *= m_gain;
		sum += (FastAbs(SinglePerlin(m_perm[i], x, y, z, m_interp)) * 2 - 1) * amp;
	}
...
{
	FN_DECIMAL sum = 1 - FastAbs(SinglePerlin(m_perm[0], x, y, z, m_interp));
	FN_DECIMAL amp = 1;
	int i = 0;
	...
	while (++i < m_octaves)
	{
		...
		amp *= m_gain;
		sum -= (1 - FastAbs(SinglePerlin(m_perm[i], x, y, z, m_interp))) * amp;
	}

	return sum;
}

FN_DECIMAL SinglePerlin2D(unsigned char offset, FN_DECIMAL x, FN_DECIMAL y,enum Interp m_interp)
{
	int x0 = FastFloor(x);
	int y0 = FastFloor(y);
	int x1 = x0 + 1;
...
{
	FN_DECIMAL sum = SingleValue(m_perm[0], x, y, z, m_inter);
	FN_DECIMAL amp = 1;
	int i = 0;
	...
	while (++i < m_octaves)
	{
		...
		amp *= m_gain;
		sum += SingleValue(m_perm[i], x, y, z, m_inter) * amp;
	}
...
{
	FN_DECIMAL sum = FastAbs(SingleValue(m_perm[0], x, y, z, m_inter)) * 2 - 1;
	FN_DECIMAL amp = 1;
	int i = 0;
	...
	while (++i < m_octaves)
	{
		...
		amp *= m_gain;
		sum += (FastAbs(SingleValue(m_perm[i], x, y, z, m_inter)) * 2 - 1) * amp;
	}
...
{
	FN_DECIMAL sum = 1 - FastAbs(SingleValue(m_perm[0], x, y, z, m_inter));
	FN_DECIMAL amp = 1;
	int i = 0;
	...
	while (++i < m_octaves)
	{
		...
		amp *= m_gain;
		sum -= (1 - FastAbs(SingleValue(m_perm[i], x, y, z, m_inter))) * amp;
	}
...
static const FN_DECIMAL F3 = 1 / 3.0;
static const FN_DECIMAL G3 = 1 / 6.0;
...
static const FN_DECIMAL F4 = (sqrt(5.0) - 1) / 4;
static const FN_DECIMAL G4 = (5 - sqrt(5.0)) / 20;
...
void GradientPerturbFractal(FN_DECIMAL* x, FN_DECIMAL *y,enum Interp m_interp,int m_octaves,FN_DECIMAL m_lacunarity,FN_DECIMAL m_gain,FN_DECIMAL m_fractalBounding,FN_DECIMAL m_gradientPerturbAmp,FN_DECIMAL m_frequency)
{
	...
	SingleGradientPerturb2D(m_perm[0], amp, m_frequency, x, y,m_interp);

	while (++i < m_octaves)
	{
		freq *= m_lacunarity;
		amp *= m_gain;
		SingleGradientPerturb2D(m_perm[i], amp, freq, x, y,m_interp);
	}
}
